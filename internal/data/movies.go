...
func ValidateMovie(v *validator.Validator, movie *Movie) {
	...
	v.Check(validator.IsUnique(movie.Genres), "genres", "must not contain duplicate values")
}
...
// The Get() method accepts a id of record to return
...
// Update() method with data race check in case of update of same record at the same time
//
...
func (m MovieModel) Update(movie *Movie) error {
	...
	if err != nil {
		switch {
		// data race occurred because of no row selected with exact version match
		case errors.Is(err, sql.ErrNoRows):
